...
function calculate(){
  ...
  $('.invoicelist-body tbody tr').each( function(){
    ...
    row.find('.sum').text( formatNumberWithSeparators(sum.toFixed(2)) );
    row.find('.tax').text( formatNumberWithSeparators(tax.toFixed(2)) );   
  });

  $('#total_price').text(formatNumberWithSeparators(total_price.toFixed(2)));
  $('#total_tax').text(formatNumberWithSeparators(total_tax.toFixed(2)));
}
